...
public sealed class DeniaBackToFar : DeniaCard
{
    ...
    public override List<(string, string)>? Localization => new CardLoc(Title: "回到远方", Description: "所有附加[gold]聚爆[/gold]的效果额外附加1层。\n黯核强化：额外层数变为2。");
    protected override async Task OnPlay(PlayerChoiceContext ctx, CardPlay play) { int amount = await TrySpendDarkCore(play) ? 2 : 1; await PowerCmd.Apply<DeniaExtraBurstPower>(ctx, Owner.Creature, amount, Owner.Creature, this); }
