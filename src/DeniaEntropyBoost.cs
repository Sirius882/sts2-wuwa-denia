...
public sealed class DeniaEntropyBoost : DeniaCard
{
    ...
    public override List<(string, string)>? Localization => new CardLoc(Title: "熵变强化", Description: "每当自己获得增益或给敌人附加减益时，获得1点[gold]格挡[/gold]，每回合最多触发6次。\n黯核强化：每次获得的[gold]格挡[/gold]+1。");
    protected override async Task OnPlay(PlayerChoiceContext ctx, CardPlay play)
    {
        int blockPerTrigger = await TrySpendDarkCore(play) ? 2 : 1;
